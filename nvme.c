		     ...
		     char **argv)
{
	...
	if (opcode & 1)
		err = nvme_write(fd, cfg.namespace_id, cfg.start_block,
				 cfg.block_count, control, dsmgmt, 0,
				 cfg.ref_tag, cfg.app_tag, cfg.app_tag_mask,
				 cfg.storage_tag, buffer_size, buffer,
		      ...
		      struct plugin *plugin)
{
	...
	err = nvme_verify(fd, cfg.namespace_id, cfg.start_block,
			  cfg.block_count, control, cfg.ref_tag, cfg.app_tag,
			  cfg.app_tag_mask, cfg.storage_tag,
			  NVME_DEFAULT_IOCTL_TIMEOUT);
	if (err < 0)
		perror("verify");
	else if (err != 0)
