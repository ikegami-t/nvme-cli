					...
					unsigned *total_size)
{
	const size_t bs = 512;
	int err = 0;
	int block = NVME_LOG_TELEM_BLOCK_SIZE;
	unsigned total = 0;
	__u32 result, len;
	void *buf = NULL;
	struct nvme_id_ctrl ctrl;
	...
	switch (data_area) {
	case 1:
		total = (le16_to_cpu(log->dalb1) + 1) * block;
		break;
	case 2:
		total = (le16_to_cpu(log->dalb2) + 1) * block;
		break;
	case 3:
		fallthrough;
	default:
		total = (le16_to_cpu(log->dalb3) + 1) * block;
		break;
	case 4:
		err = nvme_identify_ctrl(fd, &ctrl);
		if (err) {
		...
		if (err > 0) {
			...
			if (ctrl.lpa & 0x40) {
				if (((unsigned char *)buf)[1] == 1) {
					total = le32_to_cpu(log->dalb4) * bs + block;
				} else {
					fprintf(stderr,
